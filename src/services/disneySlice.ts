...
export const disneySlice = createSlice({
	...
	reducers: {
		...
		toggleLike: (state, action: PayloadAction<number>) => {
			const toggleCard = state.characters.find(
				(item) => item._id === action.payload
			)
	...
	selectors: {
		...
		selectCharactersLoading: (state) => state.loading,
		selectCharactersError: (state) => state.error,
	},
...
export const selectItemById = createSelector(
	disneySlice.selectors.selectCharacters,
	(_, id) => id,
	(news: TCharacter[], id: number) => news.find((item) => item._id === id)
)

export const selectLiked = createSelector([selectCharacters], (news) =>
	news.filter((item) => item.liked === true)
)
